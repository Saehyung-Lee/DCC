...
class ImageNet(object):
    def __init__(self, path, class_list=None):
        self.path = path
        image, label, eval_image, eval_label = self._load_data()
        if class_list is not None:
            tf = label == -1
            eval_tf = eval_label == -1
            label_set = self._class_list_to_number(class_list)
            for y in label_set:
                tf += label==y
                eval_tf += eval_label==y
            image = image[tf]
            label = label[tf]
            eval_image = eval_image[eval_tf]
            eval_label = eval_label[eval_tf]
            for j in range(len(class_list)):
                label[label==label_set[j]]=j
                eval_label[eval_label==label_set[j]]=j
        self.image = image
        self.label = label
        self.mean = np.mean(self.image, axis=(0,1,2)) / 255.
        self.std = np.std(self.image, axis=(0,1,2)) / 255.
    ...
    def _load_data(self):
        ...
        for ii, fname in enumerate(train_filenames):
            cur_images, cur_labels = self._load_datafile(os.path.join(self.path, fname))
            x_list.append(cur_images)
            y_list.append(cur_labels)
        image = np.concatenate(x_list, axis=0)
        label = np.concatenate(y_list, axis=0)
        eval_image, eval_label = self._load_datafile(
            os.path.join(self.path, eval_filename))
        return image, label, eval_image, eval_label
    ...
    def _class_list_to_number(self, class_list):
        label_set = []
        class_list_copy = class_list.copy()
        with open('./map_clsloc.txt') as f:
            for line in f:
                tok = line.split()
                if tok[2] in class_list:
                    class_list_copy.remove(tok[2])
                    label_set.append(int(tok[1])-1)
        assert len(class_list) == len(label_set), "remaining: %s"%class_list_copy
        return label_set
...
def get_dataset(dataset, data_path, imagenet_class_list=None):
    if dataset == 'MNIST':
        channel = 1
        im_size = (28, 28)
        num_classes = 10
        ...
        channel = 3
        im_size = (32, 32)
        if imagenet_class_list is not None:
            num_classes = len(imagenet_class_list)
        elif imagenet_random_classes > 0:
            num_classes = imagenet_random_classes
        else:
            num_classes = 1000
        imagenet = ImageNet(data_path, class_list=imagenet_class_list)
        mean = imagenet.mean
        std = imagenet.std
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = D(imagenet.image, imagenet.label, transform) # no augmentation
        dst_test = D(imagenet.eval_image, imagenet.eval_label, transform=transform)
        class_names = imagenet_class_list

    else:
        exit('unknown dataset: %s'%dataset)

    testloader = torch.utils.data.DataLoader(dst_test, batch_size=256, shuffle=False, num_workers=4)
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    if model == 'MLP':
        ...
        net = VGG19BN(channel=channel, num_classes=num_classes)
    elif model == 'ResNet18':
    ...
    elif model == 'ResNet34BN_AP':
        net = ResNet34BN_AP(channel=channel, num_classes=num_classes)
...
def subgrouping(features, num_classes, ipc, cpg, k, distance, random, warmup, device, cifar100coarse=False):
    if warmup:
        subgroup = np.arange(num_classes)
        return np.reshape(subgroup, [num_classes, 1])
    elif random:
        subgroup = np.random.permutation(num_classes)
        return np.reshape(subgroup, [num_classes // cpg, cpg])
    elif cifar100coarse:
        cluster = [[] for i in range(20)]
        for c in range(num_classes):
            cluster[sparse2coarse(c)].append(c)
        return cluster
    else:
        cluster_ids, _ = kmeans(X=features, num_clusters=k, distance=distance, device=device)
        r = np.arange(num_classes)
        cluster = []
        for c in range(k):
            cluster.append(r[cluster_ids == c])
        return cluster
...
def sim_log(gwr, gws, is_cos=False):
    shape = gwr.shape
    if len(shape) == 4: # conv, out*in*h*w
        gwr = gwr.reshape(shape[0], shape[1] * shape[2] * shape[3])
        gws = gws.reshape(shape[0], shape[1] * shape[2] * shape[3])
    ...
    if is_cos:
        dis = torch.sum(torch.sum(gwr * gws, dim=-1) / (torch.norm(gwr, dim=-1) * torch.norm(gws, dim=-1) + 0.000001))
    else:
        dis = torch.sum(torch.abs(torch.sum(gwr * gws, dim=-1)))
    return dis

def class_zero_match_log(gw_syn, gw_real, f_syn, f_real, args):
    gw_syn_zero = gw_syn[0]
    f_syn_zero = f_syn[0]
    dot = []
    #cos_g = []
    cos_f = []
    for c, gw_real_c in enumerate(gw_real):
        dot_c = torch.tensor(0.0).to(args.device)
        #cos_g_c = torch.tensor(0.0).to(args.device)
        for ig in range(len(gw_real_c)):
            gwr = gw_real_c[ig]
            gws = gw_syn_zero[ig]
            dot_c += sim_log(gwr, gws)
            #cos_g_c += sim_log(gwr, gws, True)
        cos_f_c = sim_log(torch.mean(f_real[c], dim=0, keepdim=True), torch.mean(f_syn_zero, dim=0, keepdim=True), True)
        dot.append(dot_c)
        #cos_g.append(cos_g_c)
        cos_f.append(cos_f_c)
    denom = sum(dot).detach().clone()
    for c in range(len(gw_real)):
        dot[c] = dot[c] / denom
        dot[c] = dot[c].cpu().detach().numpy()
        #cos_g[c] = cos_g[c].cpu().detach().numpy()
        cos_f[c] = cos_f[c].cpu().detach().numpy()
    dot = np.array(dot).reshape(1, -1)
    #cos_g = np.array(cos_g).reshape(1, -1)
    cos_f = np.array(cos_f).reshape(1, -1)
    #return dot, cos_g, cos_f
    return dot, cos_f
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args):
    ...
    print('%s Evaluate_%02d: %s = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), it_eval, s, Epoch, int(time_train), loss_train, acc_train, acc_test))
